...
- **Entries-panel UX (2026-05-07).** Two frontend fixes in `templates/index.html`: (1) delete now opens a confirmation modal with status-aware copy — drafts get a soft "Delete this draft?", Harvest-pushed entries get a red destructive "Delete from Harvest" with a warning that it will be removed from both systems; Cancel is auto-focused so a stray Enter never destroys a Harvest entry; (2) approved entries collapsed into lean one-line rows under an "Approved (N · Xh)" section header (Option C), drafts stay full-size at the top — click a row to expand, click again to collapse. 73 tests pass. Backend untouched.
